...
from ..utils.IPPerfMonitor import IPPerfMonitor
...
class RRT(PRMBase):
    ...
    def planPath(self, startList, goalList, config):
        ...
        while self.lastGeneratedNodeNumber < config["numberOfGeneratedNodes"]:
            ...
            posList = nx.get_node_attributes(self.graph,'pos').values()
            kdTree = cKDTree(posList)

            if (self.lastGeneratedNodeNumber % config["testGoalAfterNumberOfNodes"]) == 0:
                #print "testing goal"
                result = kdTree.query(checkedGoalList[0],k=1)
                if not self._collisionChecker.lineInCollision(self.graph.node[result[1]]['pos'],checkedGoalList[0]):
                    self.graph.add_node("goal", pos=checkedGoalList[0])
                    self.graph.add_edge(result[1],"goal")
                    mapping={0:'start'}
                    self.graph = nx.relabel_nodes(self.graph,mapping)
            ...
            start = np.array(self.graph.node[result[1]]['pos'])
            end   = np.array(pos)
            newPos = 0.5 * (end-start) + start
            if not self._collisionChecker.lineInCollision(self.graph.node[result[1]]['pos'],newPos):
