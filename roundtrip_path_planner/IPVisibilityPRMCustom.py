...
class VisibilityStatsHandler():
    ...
    def addVisTest(self,fr,to): # Verbindungstest ob Obstakel dazwischen ist
        self.graph.add_edge(fr, to)
        return
        

class VisPRMCustom(PRMBase):
    """Class implements a simplified version of a visibility PRM""" # Das wird geändert

    def __init__(self, _collChecker, _statsHandler = None): # Erstelle Graph und einen VisibilityStstsHandler (das oben)
        super(VisPRMCustom, self).__init__(_collChecker)
        self.graph = nx.Graph()
    ...
    @IPPerfMonitor
    def _learnRoadmap(self, ntry): # Erzeugt die Roadmap
        """
        Args:
        ...
        while currTry < ntry:
            ...
            merged = False
            for component in nx.connected_components(self.graph): # Impliciteley represents G_vis # iteriert über alle Nodes des Graphen
                found = False
                merged = False
